...
from modules.task.comment_types import CommentErrorCode
...
class TestCommentApi(BaseTestTask):
    ...
    def test_create_comment_task_not_found(self) -> None:
        ...
        response = self.make_authenticated_comment_request(
            "POST", account.id, token, task_id=non_existent_task_id, data=comment_data
        )
    ...
    def test_get_specific_comment_task_not_found(self) -> None:
        account, token = self.create_account_and_get_token()
        non_existent_task_id = "507f1f77bcf86cd799439011"
        non_existent_comment_id = "507f1f77bcf86cd799439012"

        response = self.make_authenticated_comment_request(
            "GET", account.id, token, task_id=non_existent_task_id, comment_id=non_existent_comment_id
        )
    ...
    def test_delete_comment_success(self) -> None:
        account, token = self.create_account_and_get_token()
        task = self.create_test_task(account_id=account.id)
        created_comment = self.create_test_comment(account_id=account.id, task_id=task.id, content="Delete Me")
    ...
    def test_comments_are_account_isolated_via_api(self) -> None:
        ...
        create_response = self.make_authenticated_comment_request(
            "POST", account1.id, token1, task_id=task1.id, data=comment_data
        )
        account1_comment_id = create_response.json.get("id")

        get_response = self.make_cross_account_comment_request(
            "GET", account1.id, token2, task_id=task1.id, comment_id=account1_comment_id
        )
        ...
        delete_response = self.make_cross_account_comment_request(
            "DELETE", account1.id, token2, task_id=task1.id, comment_id=account1_comment_id
        )
        ...
        verify_response = self.make_authenticated_comment_request(
            "GET", account1.id, token1, task_id=task1.id, comment_id=account1_comment_id
        )
